...
test("カプセル化実験",function(){
	...
	var KlassFactory = function(){
		...
		//ファクトリーオブジェクトを返却。ここでクロージャを使うことで、ベースクラス・サブクラス自体を隠蔽する。
		return {
    ...
    var base = KlassFactory.createKlass('111');
    equal('111',base.getAttr());
    //クラスの定義を外から参照できないので、instaneofとか使えない。モジュール化を厳密にしたい場合ならいいが、これはいまいち。
